...
# Yolo wolrd pour n'importe quel prompt
# https://docs.ultralytics.com/models/yolo-world/?h=#key-features
# voir pour sauvegarder embedding texte avant pour gagner du temps
# est ce que on peut faire du onxx ou openvino avec yolo world
# TODO: tenter de extraire avec openvino ou onnx
# en open vocabulary je peux potentiellement utiliser
# grounded dino
# https://github.com/IDEA-Research/GroundingDINO?tab=readme-ov-file
# grounded segment anything pour avoir aussi les masks
# https://github.com/IDEA-Research/Grounded-Segment-Anything
# model yolo https://docs.ultralytics.com/models/yolov8/#supported-tasks-and-modes
# pour le cpu
# open vino est beaucoup plus rapide 15 ms pour le xl
# https://docs.ultralytics.com/integrations/openvino/#intel-flex-gpu
...
class TIAM_per_prompt(TIAMScore):
    ...
    def __init__(
        ...
        super().__init__(attribute_binding, confs_for_score, min_conf_threshold)
        self.model: YOLO = YOLO(
            model_path,
        )

        self.iou_nms = iou_nms  # iou	float	0.7	Intersection Over Union (IoU) threshold for Non-Maximum Suppression (NMS). Higher values result in fewer detections by eliminating overlapping boxes, useful for reducing duplicates.

        self.augment = False  # augment	bool	False	Enables test-time augmentation (TTA) for predictions, potentially improving detection robustness at the cost of inference speed.
    ...
    def predict(
        ...
        for batch, seeds in self.stream_batch_size(images, seeds_used):
            ...
            for (idx_img, r), s in zip(enumerate(results), seeds):
                ...
                # r.save(filename=f"/gpfsscratch/rech/bum/ujm26tk/debug_yolo/img_{idx_img}.png")
                if self.seg:
                    detected_colors = []
                    percentages_colors = []
