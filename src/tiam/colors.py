...
def munsell_to_XYZ(hue, brightness, chroma):
    ...
    try:
        ...
        raise ValueError(f"error with {MRS_c}, {e}")
        ...
        for key, method in methods.items():
            for v in values:
                if one_best_example[color][key] is None:
                    one_best_example[color][key] = method(HUE[v[1]], BRIGHTNESS[v[0]], CHROMA[v[0]][v[1]])
                else:
                    one_best_example[color][key] += method(HUE[v[1]], BRIGHTNESS[v[0]], CHROMA[v[0]][v[1]])
...
row6 = (
    [14] * 4 + [10] + [8] * 2 + [6] * 7 + [8] * 2 + [10] * 4 + [8] * 4 + [6] * 2 + [8] * 2 + [10] * 2 + [12] + [10] * 9
)
row7 = (
    [10] * 2
    + [12]
...
best_example_Berlin_Kay_coord2label = {
    (i, j): label for label, coords in best_example_Berlin_Kay.items() for i, j in coords
}

# Compute one best example for each color
# one example in Lab

one_best_example_Berlin_Kay = interpolate_best_example(best_examples_per_color=best_example_Berlin_Kay)

# define cielbab_berlin_kay
cielab_Berlin_Kay = {color: v["lab"] for color, v in one_best_example_Berlin_Kay.items()}
